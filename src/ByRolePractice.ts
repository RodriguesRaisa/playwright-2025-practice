...
(async () => {
   ...
   let page:Page = await browser.newPage();
    ...
    //1.textbox should have a <label> tag or <aria-label attribute> then only we can use getByRole
    ...
    //2.In case of link use link name in getByRole
   // await page.getByRole('link',{name:'Forgotten Password'}).click();

   //h1 to h6 are headers , in that case use heading
